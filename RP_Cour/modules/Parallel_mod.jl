...
function myrange_y(t::SharedArray,p)
    idx = indexpids(t)
...
function fill_table!(t::SharedArray, pt, qt)
    ...
    for (i,y) in zip(split, years)
        ...
        t[i,1], t[i,2], t[i,3] = RP_Cournot_test(mchain)
    end
    t
end
...
# Semesters
function myrange_sem(t::SharedArray,p)
    idx = indexpids(t)
    if idx == 0 # This worker is not assigned a piece
       return 1:0, 1:0
    end
...
function fill_chain_sem!(t::SharedArray, pt, qt)
...
function fill_chain_sem_algo!(t::SharedArray, pt, qt)
    split, years = myrange_sem(t,pt)
    for (i,y) in zip(split, years)
        t[i,1,1] == 0.0 || continue # don't do previously done evals
        @show (myid(),i ,y)
...
function fill_chain_sem_alt!(t::SharedArray, pt, qt)
    split, years = myrange_sem(t,pt)
    for (i,y) in zip(split, years)
        t[i,1,1] == 0.0 || continue # don't do previously done evals
        @show (myid(),i ,y)
...
function hM_chunk!(hM, mchain, gamma)
...
function hM_chunk_alt!(hM, mchain, gamma)
   # @show (irange, jrange, trange)  # display so we can see what's happening
   ...
   for t in range, r=1:nfast
       ...
       geta[t,:]=rand() < 1.4^(val[1]) ? gtry : geta[t,:]
       hM[t,:]=+geta[t,:]./nfast
       # return hM
   end
   hM
...
# function hM_dist_chunk!(hM, mchain, gamma)
...
function shared_hM_fill!(hM, m, g, fun::Function)
    @sync begin
        for w in procs(hM)
...
function objfun_par(gamma0...; mchain=mchain_op)
...
function objfun_par_alt(gamma0...; mchain=mchain_op)
    # @unpack nsim, nfast = Par
    gamma = gamma0
    T, N, nfast = size(mchain)

    hM = SharedArray{Float64,2}(zeros(T,N))
    shared_hM_fill!(hM, mchain, gamma, hM_chunk_alt!)
...
function objfun_bbo_par_alt(gamma0; mchain=mchain_op)
...
function myrange_dec(t::SharedArray)
    idx = indexpids(t)
    if idx == 0 # This worker is not assigned a piece
       return 1:0, 1:0
    end
...
function fill_table_dec!(t::SharedArray, m)
